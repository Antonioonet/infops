...
class SuperModularModel(torch.nn.Module):
    ...
    def forward(self, data, mean_error_list):
        ...
        if not torch.is_tensor(mean_error_list):
            mean_error_list = torch.tensor(mean_error_list, dtype=torch.float32)

        mean_error_list = mean_error_list.to(device)

        selected_nodes = torch.arange(data.num_nodes, device=device)[
            mean_error_list > self.threshold
        ]
        ...
        residal_data = Data(
            x=mean_error_list[mean_error_list > self.threshold].view(-1, 1),
            edge_index=residual_edge_index,
        )
        ...
        residal_data.batch = batch
        residal_data = residal_data.to(data.x.device)

        anomaly_scores = self.smgnn(
            residal_data.x,
            residal_data.edge_index,
            residal_data.batch
        )

        anomaly_nodes_index = selected_nodes

        return anomaly_scores, residal_data, anomaly_nodes_index
