...
def main():
    ...
    output_root = args.output_dir or (
        ROOT / "artifacts/results" / f"dominant_pretrain_{date.today().isoformat()}"
    )
    ...
    columns = [
        "target_dataset", "source_datasets", "seed", "hid_dim", "epochs",
        "lr", "batch_size", "num_neigh", "pretrain_epochs",
        "calibration_nodes", "validation_nodes", "unused_nodes", "threshold",
        "calibration_macro_f1", "validation_macro_f1", "validation_auc",
        "validation_precision", "validation_recall", "pretraining_seconds",
        "scoring_seconds", "status",
    ]
        ...
        for target_dataset in args.datasets:
            ...
            for seed in args.seeds:
                ...
                row = {
                    ...
                    "source_datasets": ",".join(name for name, _ in source_data),
                    "seed": seed,
                    "hid_dim": int(hp["hid_dim"]),
                    ...
                    "calibration_nodes": int(calibration_mask.sum()),
